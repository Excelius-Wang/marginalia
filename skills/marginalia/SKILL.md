...
4. **写中文笔记。** 用 `references/note-schema.md` 的模板。重点写好招牌段落：`毒舌评论`、`作者思路重建`、`最脆弱的假设`、`最小复现实验`、`最强反例设计`、`Follow-up Research Idea`。分清"作者展示的"和"你推出的"。在 `Key Artifacts` 里给关键图/表打 `[p.N]` 页码标记（供发布时抽图），关键公式用 `$...$` 内联以便渲染。
