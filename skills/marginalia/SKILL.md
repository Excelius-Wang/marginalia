---
name: marginalia
description: 深度精读学术论文，产出结构化的中文研究笔记，带批判性分析——重建作者思路、机制层面讲清方法、找出最脆弱的假设、设计反例、提出非增量的 follow-up 研究方向，并把笔记沉淀进 git 知识库（notes/ + README 索引）。当用户给出论文标题、DOI、arXiv/OpenReview/出版社/GitHub/PDF 链接或本地 PDF，或要求 精读/阅读/总结/分析/批判/复现/归纳贡献/比较相关工作/设计反例/提出后续研究/维护论文索引，或要 read / summarize / critique / deep-read a paper、design a minimal reproduction or counterexample、propose follow-up research 时使用。适用于 ML、NLP、CV、systems、安全、区块链、生物、经济等领域。
---
...
4. **写中文笔记。** 用 `references/note-schema.md` 的模板。重点写好招牌段落：`毒舌评论`、`作者思路重建`、`最脆弱的假设`、`最小复现实验`、`最强反例设计`、`Follow-up Research Idea`。分清"作者展示的"和"你推出的"。

5. **落盘与回填。** 按 `references/git-workflow.md`：笔记写到 `notes/<领域>/【VENUE‘YEAR】short-title.md`，回填 README 索引（`TODO`→`DONE` 并链接到笔记文件）。

6. **同步到 Git。** 在 git 知识库里，只 stage 本次任务改动的文件，commit（`Add paper note: <短标题>` 或 `Update paper note: ...`），再 `git push`。无改动不空 commit；push 失败要清楚报错并保留本地 commit。
